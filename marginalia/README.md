...
- **AI 词源考古**：`scripts/vocab-daily.ts`——用你自己的 Claude 给词表续写词源随笔（500+ 篇预置语料随包附带）。需要 [bun](https://bun.sh) 和 [Claude Code CLI](https://claude.com/claude-code)：
  ```bash
  bun scripts/vocab-daily.ts 30        # 按词表序补 30 个缺口词
  bun scripts/vocab-daily.ts --word brew   # 单独考古一个词
  ```
  产物写进包内 `Resources/vocab-corpus/`，重新 build 即可见；接了 `requestArcheology` 桥 + `VocabCorpusStore.saveLive` 则可实时回投不用重装
...
```bash
git clone https://github.com/replica882/mneme-atrium
open mneme-atrium/marginalia/App/MarginaliaApp.xcodeproj
```
...
> ⚠️ 本包住在 mneme-atrium 的子目录里，SPM 不支持直接用 URL 引用子目录包——clone 后本地引用：

```bash
git clone https://github.com/replica882/mneme-atrium
```

Xcode：File → Add Package Dependencies → **Add Local...** → 选 `mneme-atrium/marginalia`；
或 Package.swift：

```swift
.package(path: "../mneme-atrium/marginalia")
// products: "Marginalia"（全套）或只要视觉 "JournalKit"
```
